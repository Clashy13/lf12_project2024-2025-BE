...
def solve(imagepath):
    try:
        ...
        for i, cluster in enumerate(crossworddata.clustersdata):
            answerlines = answers_scrapper.CrosswordScraper().scrap(
                cluster.questionlines
            )
        ...
        solved_image_path = f"solved/{orgid}.jpg"
        CrosswordModel.objects.filter(id=orgid).update(solved_image=solved_image_path)
